...
const localizedText = (label: string, name: string, ui: 'input' | 'textarea' = 'input') => ({
  type: 'object',
  label,
  name,
  fields: [
    ...
    collections: [
      {
        ...
        fields: [
          ...
          { type: 'object', name: 'sections', label: 'Sections', fields: ['food','restaurant','merch','reservations','instagram','footer'].map((name) => ({ type: 'object', name, label: name, fields: [localizedText('Eyebrow', 'eyebrow'), localizedText('Title', 'title'), localizedText('Body', 'body', 'textarea')] })) },
        ],
      },
      {
        name: 'menus',
